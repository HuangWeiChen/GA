...
def genetic_algorithm_parallel(distance_matrix, population_size, cities, num_generations, crossover_rate, mutation_rate):
    ...
    for generation in range(num_generations):
        ...
        selected_parents = selection(population, population_size // 2, fitness_scores)
...
def selection(population, selection_number, fitness_scores):
    prob = np.array(fitness_scores) / np.sum(fitness_scores)
    selected_indices = np.random.choice(len(population), selection_number, p=prob, replace=False)
